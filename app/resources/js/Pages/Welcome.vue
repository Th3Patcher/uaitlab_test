...
function randomLogoColor() {
    test.value.color = '#' + (Math.random() * 0xfffff * test.value.count).toString(16).slice(0,6);
    test.value.count++;
}
</script>
...
<template>
    ...
    <div
        ...
        @mousemove="randomLogoColor"
    >
        <div class="absolute top-0 left-0 bg-amber-50">{{test.count}}</div>
